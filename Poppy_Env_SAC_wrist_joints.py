...
class PoppyEnv(gym.Env):
    ...
    def __init__(self, goals=2, terminates=True):
        ...
        # adjusted observation space to be between :
        ...
        #    'l_elbow_y': (-45.0, 90.0),         # (-60, 90) bends the elbow
        # right arm joints :
        ...
        self.low_limits = np.array([ -0.2, -0.40, -0.2, -0.5, -0.40, 0.2, -5.0, -180, -30, -45, -180,-180, -45, ,-45], dtype=np.float32)
        self.high_limits = np.array([ 0.5,  0.40,  0.6, 0.2,  0.40, 0.6, 180, 65, 30, 90, 5, 50, 55, 90], dtype=np.float32)
    ...
    def get_observation(self):
        '''Get observations from Poppy
        Input : None
        Outputs :
            targets_obs : (x,y,z) positions of left and right end effectors
            l_joints_obs : 3 bust joints + left arm 4 joints positions (order l_shoulder_y, l_shoulder_x, l_arm_z, l_elbow_y)
            r_joints_obs : 3 bust joints + right arm joints positions   (order r_shoulder_y, r_shoulder_x, r_arm_z, r_elbow_y)
        '''
        # Get observations
    ...
    def reward(self, obs, target, threshold=0.3, alpha=2):
        ...
        reward = (l_reward**2 + r_reward**2)*10 # + l_z_reward + r_z_reward
    ...
    def step(self, action):
        ...
        # we will use only end effectors position for now, joints position could be used also
        # obs, _, _ = self.poppy_goto(action)
        self.poppy_goto(action)
        wrist_coord, l_joints_obs, l_joints_obs = self.get_observation()
        obs= np.r_[obs, l_joints_obs[3:], l_joints_obs[3:]]
