...
from utils.video_capturing import preprocess_skeletons, mooving_average , interpolate_skeletons
import os
...
class PoppyEnv(gym.Env):
    ...
    def __init__(self, goals=2, terminates=True):
        ...
        self.joints_limits = { 
                 ...
                 'bust_y': (-27.0 , 22.0),      # bends forward (>0) or backward (<0)
                 'bust_x':(-20.0, 20.0),        # leans to the left (>0) or to the right (<0)
                 ...
                 'r_arm_z': (-50.0,60.0),       # rotates the arm around the shoulder-elbow axis
                 'r_elbow_y': (-60,90),         # bends the elbow

                 'head_y': (0.0,0.0),           # fix - points the head down (>0) or up(<0) (min -20, max 20)                                   # 
                 'head_z': (0.0,0.0),           # fix - rotates the head to the left (>0) or to the right (<0)  (min -90, max 90) 
                }
        self.low_limits = np.array([lim[0] for lim in self.joints_limits.values()], dtype=np.float32)
        self.high_limits = np.array([lim[1] for lim in self.joints_limits.values()], dtype=np.float32)
    ...
    def poppy_goto(self, joints_to_move, wait_for = 3):
        ...
        joints_to_move = { 
                    'abs_z': 0.0,      # (-90.0 , 90.0)    pivots to the left (>0) or to the right (<0) around the z-axis
                    'bust_y': 0.0,     # (-27.0 , 22.0)    bends forward (>0) or backward (<0)
                    'bust_x': 0.0,     # (-20.0, 20.0)     leans to the left (>0) or to the right (<0)
                    
                    'l_shoulder_x': 0.0,   #  (0 , 180)      lifts elbow up or down
                    'l_shoulder_y': 0.0,   # (-210.0 , 65.0) moves shoulder forward or backward
                    'l_arm_z':  0.0,       # (-50.0, 60.0)   rotates the arm around the shoulder-elbow axis
                    'l_elbow_y': 0.0,      #  (-60.0, 90.0)  straightens arms (90 full extension => -60 bent elbow)

                    'r_shoulder_x': 0.0,   #  (0 , 180)      lifts elbow up or down
                    'r_shoulder_y': 0.0,   # (-210.0 , 65.0) moves shoulder forward or backward
                    'r_arm_z':  0.0,       # (-50.0, 60.0)   rotates the arm around the shoulder-elbow axis
                    'r_elbow_y': 0.0,      #  (-60.0, 90.0)  straightens arms (90 full extension => -60 bent elbow)

                    #Optionnal (will be reset to 0)
                    'head_y': 0.0,           # fix - points the head down (>0) or up(<0) (min -20, max 20)                                   # 
                    'head_z': 0.0,           # fix - rotates the head to the left (>0) or to the right (<0)  (min -90, max 90) 
                    }
        Out put : None
        ...
        for m in poppy.motors:
            if m.name in joints_to_move:
                m.goto_position(joints_to_move[m.name], wait=False) # wait=False to allow parallel movements
    ...
    def get_observation(self):
        # Get observations
        targets_obs = [self.poppy.l_arm_chain.position, self.poppy.r_arm_chain.position]
    ...
    def get_poppy_skeletons(skeletons, self.topology, self.poppy_lengths):
        ...
        quaternions = find_quaternions(self.topology, source_offsets, skeletons)

        # Use these quaternions in the forward kinematics with the Poppy skeleton
        target_offsets = torch.zeros(n_frames, n_joints, 3)
        target_offsets[:, :, -1] = self.poppy_lengths.unsqueeze(0).repeat(n_frames, 1)
    ...
    def get_targets_from_skeleton(self, target_skeletons, indices=[13, 16]):
        ...
        Inputs :
            ...
            - indices = [8, 10, 13, 12, 11, 16, 15] # Chest, head, left hand, left elbow, left shoulder, right hand, right elbow
            - indices = = [13, 12, 16, 15] # left hand, left elbow, right hand, right elbow
        Output :
    ...
    def reset(self, seed=None, **kwargs):
        ...
        for m in self.poppy.motors:
            m.goto_position(joint_pos[m.name], 1, wait=Flase) # wait=False to allow parallel movements
    ...
    def reward(self, obs, target, threshold=0.3, alpha=10):
        ...
        l_dis = np.linalg.norm(obs[3:] - np.array(self.target[3:])
        r_dis = np.linalg.norm(obs[0:3] - np.array(self.targets[0:3])
    ...
    def step(self, action):
        ...
        obs, _ , _ = self.poppy_goto(action) # we will use only end effectors position for now, 
    ...
    def get_target_from_video(self, video , path):
        ...
        self.skeletons = preprocess_skeletons(self.skeletons, topology, ref_joint = 0 , alpha= np.pi/4 , smoothing_n=4, interpolation_factor = 1 ) #Anaele : il faut faire le preprocessing
