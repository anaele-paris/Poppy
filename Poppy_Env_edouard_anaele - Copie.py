...
class PoppyEnv(gym.Env):
    ...
    def reset(self, seed=None, **kwargs):
        ...
        if self.target_loaded == False:
            ...
            self.num_steps = self.targets.shape[0] # must be set to self.targets.shape[0] when target is defined
    ...
    def poppy_goto_dict(self, joints_to_move, wait_for=3):
    ...
    def poppy_goto_(self, joints_to_move, wait_for=3):
        for i, m in enumerate(self.poppy.motors):
            if not np.isnan(joints_to_move[i]):
                # wait=False to allow parallel movements
    ...
    def get_observation(self):
        ...
        # get joints observed position : bust (3), head (2), left arm (4), right arm (4)
        joints_pos = np.r_[l_joints_obs[:3],self.poppy.head_y.present_position, self.poppy.head_z.present_position, l_joints_obs[3:], r_joints_obs[3:]]
    ...
    def step(self, action):
        ...
        obs, self.joints_pos = self.get_observation()
        joints_str = ", ".join([f"{j:.1f}" for j in self.joints_pos]) # for printing in a compact way
        ...
        reward = self.reward(obs, self.targets[self.current_step],joints = self.joints_pos)
