...
class PoppyEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # decouple the action into left-arm and right-arm.
        action_l = action[0]
        action_r = action[1]

        if self.current_step > 125:  # the left-arm stage
            # fix the right arm
        ...
        if self.current_step <= 125:
            dis = np.linalg.norm(
                obs[3:] - np.array(self.targets[self.current_step].flatten())[3:])
        else:
            dis = np.linalg.norm(
                obs[0:3] - np.array(self.targets[self.current_step].flatten())[0:3])
    ...
    def moving_average(self, a, n=3):
        ...
        repeat_shape[0] = n//2
        a = torch.cat([a[:1].repeat(*repeat_shape), a,
                      a[-2:].repeat(*repeat_shape)])
    ...
    def interpolate_targets(self, targets, factor=1):
        ...
        for i in range(new_targets.shape[0]):
            ...
            new_targets[i] = before_coef * targets[before_id] + \
                after_coef * targets[after_id]
    ...
    def targets_from_skeleton(self, source_positions, topology, poppy_lengths):
        ...
        for child, parent in enumerate(topology):
            source_lengths[:, child] = torch.sqrt(
                torch.sum(
                    (source_positions[:, child] -
                     source_positions[:, parent])**2,
                    axis=-1
                )
            )
        ...
        quaternions = find_quaternions(
            topology, source_offsets, source_positions)

        # Re-orient according to the pelvis->chest orientation
        base_orientation = quaternions[:, 7:8].repeat(
            1, n_joints, 1).reshape(batch_size*n_joints, 4)
        ...
        target_offsets[:, :, -
                       1] = poppy_lengths.unsqueeze(0).repeat(batch_size, 1)
        ...
        rotation = torch.Tensor(
            [np.cos(alpha/2), 0, 0, np.sin(alpha/2)]).unsqueeze(0).repeat(batch_size*n_joints, 1)
        quaternions = quaternions.reshape(batch_size*n_joints, 4)
        quaternions = batch_quat_left_multiply(
